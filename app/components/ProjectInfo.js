...
import user_img from "../data/images/user_img.jpg";
import project_img from "../data/images/project_img.jpg";
import frame_hand from "../data/images/frame_hand.png"

export const ProjectInfo = ({ title,owner, about, taken, goal, left }) => {
    const styles = {
        project_size: {
            minHeight: "600px",
            borderRadius: "10px"
    ...
    return (
        <div className={styles.page}>
            <aside className={styles.left}>
                <img src={project_img} className={styles.project_class} style={styles.project_size}></img>
            </aside>
            <section className={styles.right}>
                <h2 className={styles.prof_title}>{title}</h2>
                <div id="profile" className={styles.short_profile}>
                    <img src={user_img} style={styles.profile_size}></img>
                ...
                <div id='inProject' className={styles.inproject}>
                    ...
                    <div id='intoProjectRight' className={styles.intoright}>
                        <ul className={styles.ul}>
                            <li className={styles.raised_li}>Raised:<br></br><h4 className={styles.numbers}>${taken}</h4></li>
                            <li className={styles.goal_li}>Goal:<br></br><h4 className={styles.numbers}>${goal}</h4></li>
                        </ul>
                        <div id='line' className={styles.percent}>
                            <div id='percentage' className={styles.onpercent}></div>
                        </div>
                        <span className={styles.left_day}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8 -mt-2 me-2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
                        </svg>
                            {left} days left</span>
                    </div>
                </div>
                <a className={styles.button_pos}><button className={styles.button}>Fund this project</button></a>
            </section>
        </div>
    )
}
